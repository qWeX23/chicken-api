...
class ChickenFactsSheetRepository(
    ...
    fun append(record: ChickenFactsRecord) {
        val row = listOf(
            ...
            record.outcome.name,
            record.factText?.length ?: 0,
            record.factText.orEmpty(),
    ...
    fun fetchLatestChickenFact(): ChickenFactsRecord? {
        ...
        for (row in rows.asReversed()) {
            ...
            if (record != null && record.outcome == AgentRunOutcome.SUCCESS && !record.factText.isNullOrBlank()) {
                return record
            }
        }
    ...
    private fun mapRowToRecord(row: List<Any>): ChickenFactsRecord? {
        ...
        val factText = row.stringAt(6, trim = false)
        val sourceUrl = row.stringAt(7)
